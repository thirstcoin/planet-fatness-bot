...
@flask_app.route("/")
def home(): return "Planet Fatness: Lab & Kitchen Online 🧪🍔", 200
...
# ==========================================
# 4. BACKGROUND TASKS (8PM RESET & SNAPSHOT)
# ==========================================
async def automated_reset_task(application):
    while True:
        now_utc = datetime.utcnow()
        if now_utc.hour == 1 and now_utc.minute == 0: # 8PM EST (01:00 UTC)
            try:
                conn = get_db_connection(); cur = conn.cursor()
                
                # 📸 Capture Snapshot BEFORE Reset
                snapshot_queries = [
                    ('DAILY PHATTEST', 'daily_calories'),
                    ('TOP HACKER', 'daily_clog')
                ]
                
                for label, col in snapshot_queries:
                ...
                conn.commit(); cur.close(); conn.close()
                logger.info("🚨 8PM Reset & Snapshot Complete.")
...
async def gift(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice not in ['protein', 'poison']: return await update.message.reply_text("❓ Choose `protein` or `poison`.")
...
async def trash_gift(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit(); cur.close(); conn.close()
    await update.message.reply_text("🚮 **SCRAPPED:** Paid 100 Cal fee to dump the package.")

# ==========================================
# 7. STATUS & LEADERBOARDS (BULLETPROOF)
# ==========================================
async def daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """BULLETPROOF: Pulls anyone with calories > 0. Reset task handles the timing."""
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("""
...
async def winners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = cur.fetchall(); cur.close(); conn.close()
    if not rows: return await update.message.reply_text("📜 Hall of Fame is currently empty.")
